-- vanilla 1.12 + OctoWoW
--- .hcmessages 60 		turns off all death message below 60
--- alt+0177 = ±  (+-)

OctoChatColorsVer = 1.0
local occGuildBrackets = 1
...
local OctoChatColorsHooked = false
local grip = CLRED.." RIP"..CRED.." :("
local gripmsg = false
local OctoChatColors_Names = {}
local OctoChatColors_Level = {}
local GSnum = 0
local CSV = ""
...
CSV=CSV..",main tank=MainTank,octo wow=OctoWoW,alterac valley=AlteracValley,warsong gulch=WarsongGulch,need mt=Need:MT,need ot=Need:OT,project epoch=project:Epoch,project ascension=project:Ascension"
...
CSV = "wts,wtb,wtt,octowow,turtlewow,twow,guild,guildleader,github,guild-invite,leveling-guild,leveling-guilds,tent,tents,pve,macro,macros,google,wiki,SR,ambershire,nordanaar,vendor,vendors,bijou,bijous,raid,raids,raiding,raiders,gardening,rmt"
...
local OCCcount = table.getn(chReplace1) + table.getn(chatUP) + table.getn(chLocBig) + table.getn(chLocation) + table.getn(chGreen) + table.getn(chRed) + table.getn(chBlue) + table.getn(chLGreen) + 45
DCFmsg(CROLEGREEN.."OctoChatColors loaded "..CYELLOW..OCCcount..CROLEGREEN.." highlight-words!")

function OCCHighlightStrs (message)
...
function OctoChangeGuildChat (message)
	...
	if ((a and c and a<3 and b<c) or (e and g and e<3 and f<g)) and (not string.find(message,"cleanchat")) then
		...
		if level then 
			...
			hColor = OctoChatColors_GetClassColor(string.upper(hClass))
		end
	...
	message = OCCHighlightStrs(message)
	return message
end

function OctoChangeSystem (message)
	local wasokk = false
	...
	if message then	
		...
		if strsub(message,1,9)=="A tragedy" then 
			...
			if not (string.find(message," natural") or string.find(message," burned") or string.find(message," drowned") or string.find(message,"in PvP")) then
				...
				if not level then _,level = OctoChatColors_ClassData(string.upper(hName)); if level then gReadRoster(); level,hClass,hZone,hNote,_,oNote = GetGuildMemberInfo(hName) end end
		...
		elseif (strsub(message,-8)=="ey face.") and (string.find(message,"Hardcore m") or string.find(message,"immortality")) then 
			...
			if e then 
				...
				level,hClass,hZone,hNote,_,oNote = GetGuildMemberInfo(hName)
				if not level then 
					_,level = OctoChatColors_ClassData(string.upper(hName))
					if level then gReadRoster(); level,hClass,hZone,hNote,_,oNote = GetGuildMemberInfo(hName) end
				end
				if level then hColor = OctoChatColors_GetClassColor(string.upper(hClass)) else hColor=CLGRAY; hClass=""; hZone=""; hNote="" end
		...
		elseif (strsub(message,-9)=="Immortal!") then 
			...
			if a and b and c then
				...
				if level then hColor = OctoChatColors_GetClassColor(string.upper(hClass)) else hColor=CLGRAY; hClass=""; hZone="" end
		...
		elseif (strsub(message,-13)=="no Challenge!") then 
			...
			if a then
				...
				level,hClass,hZone,hNote,_,oNote = GetGuildMemberInfo(hName)
				if hNote then hNote=CGRAY.." ("..hNote..")|r"; else hNote="" end 
				if not level then _,level = OctoChatColors_ClassData(string.upper(hName)); if level then gReadRoster(); level,hClass,hZone = GetGuildMemberInfo(hName) end end
				if level then hColor = OctoChatColors_GetClassColor(string.upper(hClass)) else hColor=CLGRAY; hClass=""; hZone="" end
...
function gReadRoster()
	...
	for i = 1, numGuild do
		...
		if( class and name ) then OctoChatColors_ClassData(name, class, level) end
	end
end
...
function GShowGNInfo(hName)
	if hName then
		...
		if hLevel then
			GSnum = GSnum + 1
			local hColor = OctoChatColors_GetClassColor(string.upper(hClass))
...
function OCC_SlashCommandHandler(arg)
	gkiir("/occ:  '"..arg.."'")
	if arg and arg~="" then
		...
		gkiir("OctoChatColors /occ commands:")
...
function OctoChatColors_OnLoad() 
	this:RegisterEvent("GUILD_ROSTER_UPDATE")
	this:RegisterEvent("VARIABLES_LOADED")
	this:RegisterEvent("PLAYER_LOGIN")
	this:RegisterEvent("PLAYER_ENTERING_WORLD")
	SlashCmdList["OCC"] = OCC_SlashCommandHandler
	SLASH_OCC1 = "/occ"
	SLASH_OCC2 = "/OctoChatColors"
...
function OctoChatColors_OnEvent(event)
	if ((event == "PLAYER_ENTERING_WORLD") or (event == "PLAYER_LOGIN")) and (OctoChatColorsHooked==false) then 
		OctoChatColorsHooked = true
		OctoChatColors_OrigChatFrame_OnEvent = ChatFrame_OnEvent
		ChatFrame_OnEvent = OctoChatColors_ChatFrame_OnEvent
	end
...
function OctoChatColors_ChatFrame_OnEvent(event)
	if (event == "CHAT_MSG_SYSTEM") then
		local sysresult = OctoChangeSystem(arg1)
		if sysresult==nil or sysresult=="" then return
		else this:AddMessage(sysresult); return end
	elseif (event == "CHAT_MSG_CHANNEL") then
		local ch = string.lower(arg9 or "")
		if ch == "world" or string.find(ch, "trade") or string.find(ch, "general") or string.find(ch, "looking") or string.find(ch, "recruitment") or string.find(ch, "localdefense") then 
			arg1 = OCCHighlightStrs(arg1)
			local chanID = "?"
			if arg4 then
			...
			local pColor = OctoChatColors_ClassData(playerName)
	...
	elseif (event == "CHAT_MSG_GUILD") then
		arg1 = OctoChangeGuildChat(arg1)
	end
	OctoChatColors_OrigChatFrame_OnEvent(event)
end

function OctoChatColors_ClassData(arg2, class, level )
	if arg2 then arg2 = string.upper(arg2) end
	if class then class = string.upper(class) end
	if not OctoChatColors_Names then OctoChatColors_Names = {} end
	if not OctoChatColors_Level then OctoChatColors_Level = {} end
	if not arg2 then return "" end

	if not class then
		for name, color in OctoChatColors_Names do
			if name == arg2 then 
				if pfUI and pfUI.chat then if pfUI.chat.classcolor then if pfUI.chat.classcolor~=1 then color=CGUILD end end end
				return color, OctoChatColors_Level[arg2]
			end
		end
	end
    local found
    for name, color in OctoChatColors_Names do
		if name == arg2 then 
			if OctoChatColors_Level[name] == level then
				found = true
			end
		end
   	end
	if not found then
		local color = OctoChatColors_GetClassColor( class )
		OctoChatColors_Names[arg2] = color
		if level then
			OctoChatColors_Level[arg2] = level
		else
			OctoChatColors_Level[arg2] = 0
		end
	end
	return ""
end
...
function OctoChatColors_GetClassColor( class )
